...
| `1`       | `GENERIC`            | Catch-all for errors not (yet) assigned a dedicated code. Always the safe fallback.      | `GENERIC`, plus any unmapped code (e.g. `DOMAIN_NOT_ALLOWED`, `KEY_NOT_FOUND`, `SECRET_RESOLUTION_FAILED`, `PLUGIN_FAILED`, `PLUGIN_TIMED_OUT`) | `state load <missing>`; allowlist denial; plugin command raises or times out.    |
...
| `8`       | `VALIDATION_FAILED`  | A caller-side validation guard rejected the request. Covers the whole `VALIDATION_FAILED` family — agents can branch on `$? == 8` for any validation failure without caring which specific guard tripped. | `VALIDATION_FAILED`, `INVALID_SELECTOR_REF`, `INVALID_STATE_NAME`, `INVALID_DSL_USAGE`, `INVALID_FORMAT_VERSION`, `INVALID_ARGUMENT` | `click` with no selector or ref; `state save "bad name!"`; flow DSL missing a required block. |
