...
| `PLUGIN_FAILED` | An uncaught exception escaped a plugin command registered via `Browserctl.register_command`. The daemon catches it, returns this code with the plugin name in `context.plugin`, and stays healthy. | A plugin block raises `RuntimeError`; a plugin returns malformed data that breaks downstream serialisation. | Check the plugin's logs; the daemon caught an uncaught exception from the plugin and is otherwise healthy. | [`1`](exit-codes.md) (falls through to `GENERIC`) | [codes.rb](../../lib/browserctl/error/codes.rb) |
| `PLUGIN_TIMED_OUT` | A plugin command exceeded its per-invocation timeout (default 30s, set via `timeout:` on `register_command`; opt out with `timeout: nil`). | A plugin block sleeps or blocks past its timeout. | Increase the plugin's `timeout:` on register_command, or pass `timeout: nil` to opt out (not recommended). | [`1`](exit-codes.md) (falls through to `GENERIC`) | [codes.rb](../../lib/browserctl/error/codes.rb) |
