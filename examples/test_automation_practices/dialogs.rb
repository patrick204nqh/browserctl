...
Browserctl.workflow "test_automation_practices/dialogs" do
  ...
  step "accept a confirm — page stays responsive" do
    ...
    sleep 0.3
    res = client.evaluate("main", "1 + 1")
    assert res[:result] == 2, "expected page to be responsive after accepting confirm"
  end
  ...
  step "dismiss a confirm — page stays responsive" do
    ...
    sleep 0.3
    res = client.evaluate("main", "1 + 1")
    assert res[:result] == 2, "expected page to be responsive after dismissing confirm"
  end
  ...
  step "accept a prompt with text — page stays responsive" do
    ...
    sleep 0.3
    res = client.evaluate("main", "1 + 1")
    assert res[:result] == 2, "expected page to be responsive after accepting prompt"
